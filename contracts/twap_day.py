...
//创建表来保存结果
s_twap = table(`600000 as SecurityID, 0 as twap);
b_twap = table(`600000 as SecurityID, 0 as twap);
...
for(date in sub_list){
	...
	//计算该日各股票的twap
	s_res = select SecurityID, Price, Qty, TickTime from s_out;
	res = cal_twap(s_res);
	//外连接
	s_twap = out_join(s_twap, res, date, `res_twap);
	
	//买方
	b_out = buy_order(data);
	//计算该日各股票的twap
	b_res = select SecurityID, Price, Qty, TickTime from b_out;
	res = cal_twap(b_res);
	//外连接
	b_twap = out_join(b_twap, res, date, `res_twap);
	}
...
drop!(b_twap, `twap);
    ...
    # 脚本输出
    s_twap = s.run("s_twap")
    res = s_twap.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/sell/twap_day.csv"
    if old_exist:
    ...
    b_twap = s.run("b_twap")
    res = b_twap.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/buy/twap_day.csv"
    if old_exist:
        old = pd.read_csv(file_path, index_col=0)
        new = pd.concat([old, res])
    else:
