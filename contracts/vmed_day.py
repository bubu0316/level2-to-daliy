...
//创建表来保存结果
s_vmed = table(`600000 as SecurityID, 0 as vmed);
b_vmed = table(`600000 as SecurityID, 0 as vmed);
...
for(date in sub_list)
{
	...
	//计算vmed
	temp = select sum(Qty) as weight from s_out group by SecurityID, Price order by SecurityID, Price asc;
	res = cal_med(temp, `vmed);
	//外连接
	s_vmed = out_join(s_vmed, res, date, `res_vmed);

	//买方
	b_out = buy_order(data);
	//计算vmed
	temp = select sum(Qty) as weight from b_out group by SecurityID, Price order by SecurityID, Price asc;
	res = cal_med(temp, `vmed);
	//外连接
	b_vmed = out_join(b_vmed, res, date, `res_vmed);
}
...
drop!(b_vmed, `vmed);
    ...
    # 脚本输出
    s_vmed = s.run("s_vmed")
    res = s_vmed.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/sell/vmed_day.csv"
    if old_exist:
    ...
    b_vmed = s.run("b_vmed")
    res = b_vmed.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/buy/vmed_day.csv"
    if old_exist:
        old = pd.read_csv(file_path, index_col=0)
        new = pd.concat([old, res])
    else:
