...
//创建表来保存结果
s_vwap = table(`600000 as SecurityID, 0 as vwap);
b_vwap = table(`600000 as SecurityID, 0 as vwap);
...
for(date in sub_list)
{
	...
	//外连接
	s_vwap = out_join(s_vwap, res, date, `res_vwap);
	...
	res = select SecurityID, wavg(Price, sum_Qty) as vwap from Q group by SecurityID order by SecurityID;
	//外连接
	b_vwap = out_join(b_vwap, res, date, `res_vwap);
}
...
drop!(b_vwap, `vwap);
    ...
    # 脚本输出
    s_vwap = s.run("s_vwap")
    res = s_vwap.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/sell/vwap_whole_day.csv"
    if old_exist:
    ...
    b_vwap = s.run("b_vwap")
    res = b_vwap.set_index("SecurityID").rename(columns=lambda x: x.replace('date_', '')).rename(columns=lambda x: x.replace('_', '-')).T.rename_axis("date")

    file_path = path + "contracts/buy/vwap_whole_day.csv"
    if old_exist:
        old = pd.read_csv(file_path, index_col=0)
        new = pd.concat([old, res])
    else:
